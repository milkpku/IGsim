  ...
  DerivedE&  energy)
{
  ...
  if (Mu.size() == V.rows())
  {
    ...
    DerivedMu Tmu = Proj * Mu;
    DerivedLam Tlam = Proj * Lam;

    elastic_neohookean(V, T, Bm, W, Tmu, Tlam, energy);
  }
  else if (Mu.size() == T.rows())
  {
  ...
  Eigen::PlainObjectBase<DerivedF>& f)
{
  ...
  if (Mu.size() == V.rows())
  {
    ...
    neohookean_model(V, T, Bm, W, Tmu, Tlam, energy, f);
  }
  else if (Mu.size() == T.rows())
  {
    typedef Eigen::Matrix3d Mat3;
    ...
    for (int i = 0; i < T.rows(); i++)
    {
      ...
      /* accumulate forces */
      Mat3 H = - W(i) * P * Bm[i].transpose();
      H = H.transpose();  /* change to be row wise force */
      f.row(T(i, 0)) += H.row(0);
      f.row(T(i, 1)) += H.row(1);
      f.row(T(i, 2)) += H.row(2);
  ...
  Eigen::SparseMatrix<ScalarK>& K)
{
  ...
  if (Mu.size() == V.rows())
  {
    ...
    neohookean_model(V, T, Bm, W, Tmu, Tlam, energy, f, K);
  }
  else if (Mu.size() == T.rows())
  {
    typedef Eigen::Matrix3d Mat3;
    ...
    for (int i = 0; i < T.rows(); i++)
    {
      ...
      for(int j = 0; j < 4; j++)
        for(int k = 0; k < 3; k++)
        {
          Mat3 dH = - W(i) * dP[count++] * Bm[i].transpose();
          dH = dH.transpose(); /* change to be row wise force */
		...
		Eigen::SparseMatrix<ScalarLam>& Klam)
{
  ...
  if (Mu.size() == V.rows())
  {
    ...
    neohookean_model(V, T, Bm, W, Tmu, Tlam, energy, f, K, Kmu, Klam);
  ...
  else if (Mu.size() == T.rows())
  {
    ...
    for (int i = 0; i < T.rows(); i++)
    {
      ...
      Mat3 dHmu = - W(i) * dPmu * Bm[i].transpose();
      dHmu = dHmu.transpose();
      ...
      Mat3 dHlam = - W(i) * dPlam * Bm[i].transpose();
      dHlam = dHlam.transpose();
      /* compute dH -> df, j = 0, 1, 2 */
      for (int j = 0; j < 3; j++)
        for (int k = 0; k < 3; k++)
        {
